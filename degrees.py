...
def shortest_path(source, target):
    ...
    while True:
        if frontier.empty():
            raise Exception("no path connect those two actors")

        #Select a node from the frontier to explore
        ...
        #Mark the actor as explored
        explored.add(node.state)
        num_steps += 1

        #Search for the neighbors of the source
